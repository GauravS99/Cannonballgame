...
public class Cannonball_Game extends PApplet {
...
public void mouseClicked(){  //handles changing stages
...
public void  guiHandler(){ //handles all gui elements while the game is being run
  
 textSize(20);
  
  fill(20,247, 255);
  text("Score :" + marioHits,20.0f,40.0f);
  text("Score :" + bowserHits, (float)(LENGTH_OF_SCREEN - 150), 40.0f); 
 
  fill(0);
  text("Velocity :" + marioV,20.0f,80.0f);
  text("Velocity :" + bowserV, (float)(LENGTH_OF_SCREEN - 150), 80.0f);
...
public void fireHandler() { //handles all aspects of shooting
...
public void rotationHandler() { //handlles all rotation
...
public void switchBoolean(boolean pressed) {   //handles all the key presses
...
public int inBounds(int cannonNum) {      // gives 1 if the specified tank is out of bounds on the left boundry(different for each tank), 2 if out of bounds on right boundry (different for each tank)
...
class Cannonball {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Cannonball_Game" };
    if (passedArgs != null) {
